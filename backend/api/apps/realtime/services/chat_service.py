...
class ChatService(BaseService[Chat, ChatRepository]):
    ...
    def start_direct(self, dto: ChatCreateDTO) -> tuple[Chat, bool]:
        ...
        if existing is not None:
            ...
            if other_p is not None and other_p.status == ChatParticipant.Status.JOINED:
                return existing, False
            with transaction.atomic():
                if other_p is None:
                    self.participants.add(
                        chat_id=existing.id, user_id=other.id,
                        status=ChatParticipant.Status.JOINED if gate.auto_accept else ChatParticipant.Status.PENDING,
                    )
                else:
            ...
            chat = existing
            is_new = False
            recipient_status = ChatParticipant.Status.JOINED if gate.auto_accept else ChatParticipant.Status.PENDING
        else:
            with transaction.atomic():
        ...
        broadcast_to_user_sync(
            other.id,
            ChannelsEvent.CHAT_REQUEST_ACCEPTED if recipient_status == ChatParticipant.Status.JOINED else ChannelsEvent.CHAT_REQUEST_NEW,
    ...
    def start_chat_and_send(self, dto: ChatStartDTO) -> tuple[Chat, Message, bool]:
        ...
        if not is_group:
            other = others[0]
            from ..serializers import ChatSerializer
            other_p = chat.participants.filter(user_id=other.id, status=ChatParticipant.Status.JOINED).exists()
            broadcast_to_user_sync(
                other.id,
    ...
    def accept_chat(self, dto: ChatActionDTO) -> Chat:
        ...
        if me.status == ChatParticipant.Status.JOINED:
            return chat
        if me.status != ChatParticipant.Status.PENDING:
            raise ValidationError("There is no pending request for this chat.")
        with transaction.atomic():
            self.participants.update_status(me, status=ChatParticipant.Status.JOINED)
            self.repository.touch_last_message(chat, timezone.now())
    ...
    def send_message(self, chat_id: uuid.UUID, dto: MessageSendDTO) -> Message:
        logger.debug(f"Sending message to chat {chat_id} with message {dto}")
        self._require_actor()
        chat = self.repository.get_with_participants(chat_id)
        if chat is None:
            raise ValidationError("Chat not found.")
    ...
    def mark_seen(self, dto: ReceiptDTO) -> None:
        ...
        if chat is None:
            raise ValidationError("Chat not found.")
        me = self.participants.get(chat.id, self.actor.id)
        if me is None or me.status not in (ChatParticipant.Status.JOINED, ChatParticipant.Status.PENDING):
            raise PermissionDenied("You are not a member of this chat.")
