...
const getChatTitle = (chat: ChatResponse, currentUserId?: string) => {
    ...
    return chat.name ?? chat.participants.map((p) => p.user.username).join(", ");
};
        ...
        >
            {chat.type === "DIRECT" ? (
                <Avatar className="shrink-0" size="lg">
                    <AvatarImage src={otherParticipants[0]?.user.avatar ?? chat.avatar ?? undefined} alt={title} />
                ...
                <AvatarGroup className="shrink-0">
                    {otherParticipants.slice(0, 2).map((p) => (
                        <Avatar key={p.id}>
                            <AvatarImage src={p.user.avatar ?? undefined} alt={p.user.username} />
                            <AvatarFallback className="text-primary text-sm">
