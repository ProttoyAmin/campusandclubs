...
> {
  ...
  async getAll(params?: Record<string, unknown>): Promise<TResult[]> {
    const response = await this.client.get<ApiResponse<TResult[]>>(
      this.endpoint,
      { params },
    );
  ...
  async create(data: TCreateDTO): Promise<TResult> {
    const response = await this.client.post<ApiResponse<TResult>>(
      this.endpoint,
      data,
      {
        headers: {
  ...
  async update(id: string, data: TUpdateDTO): Promise<TResult> {
    const response = await this.client.patch<ApiResponse<TResult>>(
      `${this.endpoint}${id}`,
      data,
    );
    return response.data as TResult;
